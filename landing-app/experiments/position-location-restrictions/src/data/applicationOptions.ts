...
export type PositionOptionConfig = {
  ...
  allowedStreetsByLocation?: Record<string, string[]>;
};
  ...
  kitchen: {
    positionOptions: [
      ...
      { label: "სუშის მზარეული", availableDay: true, availableNight: false, 
        allowedLocations: ["თბილისი"], 
        allowedStreetsByLocation: { "თბილისი": ["მარშალ გელოვანის გამზირი #22", "ილია ჭავჭავაძის გამზირი #37"]} 
      },
...
const resolvePositionOptions = (
  ...
  options.map((option) => ({
    ...
    allowedLocations: Array.from(
      new Set([
        ...(option.allowedLocations ?? fallbackLocations),
        ...Object.keys(option.allowedStreetsByLocation ?? {}),
      ]),
    ),
    allowedStreetsByLocation: option.allowedStreetsByLocation,
  }));
...
export const getApplicationOptions = (
  ...
  const fallbackAllowedLocations = fallback.allowedLocations ?? fallback.locationOptions;

  return {
    positionOptions: resolvePositionOptions(
      fallback.positionOptions,
      shift,
      fallbackAllowedLocations,
    ),
